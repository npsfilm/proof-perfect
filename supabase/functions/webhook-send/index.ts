...
serve(async (req) => {
  ...
  try {
    ...
    const subjectField = gallery.salutation_type === 'Du' ? 'email_send_subject_du' : 'email_send_subject_sie';
    const bodyField = gallery.salutation_type === 'Du' ? 'email_send_body_du' : 'email_send_body_sie';
